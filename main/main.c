                                ...
                                int32_t event_id, void* event_data)
{
    ...
    // {
    //     ESP_LOGI(TAG, "WIFI 寄了");
    // }
    // else {
    //     ESP_LOGI(TAG, "WIFI READY TO SCAN!");
    // }
...
void task_wifi_scan(void *pvParam)
{
    /**
     * @description: 等待来自event_handler发任务通知，等到通知前阻塞任务
     * @param pdTRUE:xClearCountOnExit
     * @param portMAX_DELAY:xTicksToWait
     * @return {*}
     */    
    ...
    for(int i=0; i<num; i++)
    {
        ...
        ESP_LOGI(TAG, "【WIFI%2d】",i+1);
        ESP_LOGI(TAG, "WIFI_NAME:%s",records[i].ssid);
        ESP_LOGI(TAG, "WIFI_INTENSITY:%d",rssi_level);
        ESP_LOGI(TAG, "WIFI_TYPE:%d",records[i].authmode);
    }
    ESP_LOGI(TAG, "ALL TASK FINISHED! BYE!");
    vTaskDelete(wifi_scan_handle);
}
...
{
    /**
     * @description: Create a WIFI_SCAN_TASK by RTOS
     * @param TaskCodeName
     * @param Name_It almost_useless
     * @param Stack_Depth
     * @param Param_To_Task
     * @param Priority
     * @param Task_Handle
     * @return {*}
     */
    // xTaskCreate(task_wifi_scan, "wifi_scan" , 4096, NULL, 1, &wifi_scan_handle);
